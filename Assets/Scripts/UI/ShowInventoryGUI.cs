...
public class ShowInventoryGUI : MonoBehaviour
{
    ...
    private List<Container> GetAllContainersAroundTarget(float radius, out List<(Tile tile, Container container)> tileContainerPairs)
    {
        tileContainerPairs = new List<(Tile, Container)>();
        ...
        Vector2 center = targetToShow.transform.position;
        ...
        if (worldHandler != null)
        {
            for (int x = minX; x <= maxX; x++)
            {
                for (int y = minY; y <= maxY; y++)
                {
                    ...
                    {
                        containers.Add(tileContainer);
                        tileContainerPairs.Add((tile, tileContainer));
                    }
                }
            }
        }
